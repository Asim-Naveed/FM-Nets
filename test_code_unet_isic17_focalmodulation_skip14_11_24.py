...
Original file is located at
    https://colab.research.google.com/drive/1UFFyQEcTwLJxZppDami7AlC_WJ-pIFxn
"""
...
import os
!cp '/content/drive/MyDrive/segmentation_codes/ISIC2017_segmentation_data.zip' '/content/'
...
model.compile(optimizer=Adam(learning_rate = 0.00025), loss=bcc_dice_coef_loss, metrics=["accuracy"])
model.summary()


import os
!cp '/content/drive/MyDrive/segmentation_codes/ISIC2017_Segmentation_Test_Sorted.zip' '/content/'
!unzip '/content/ISIC2017_Segmentation_Test_Sorted.zip' -d '/content/' &> /dev/null
os.remove('/content/ISIC2017_Segmentation_Test_Sorted.zip')
...
def load_data(path, data_name):
    ...
    if data_name == 'a':
        imgs_list = []
        masks_list = []
        it = os.listdir(os.path.join(path,'sorted images'))
        items = sorted(it, key=lambda x: int(os.path.splitext(x)[0]))
        for item in items:
            imgs_list.append(np.array(Image.open(os.path.join(path,'sorted images',item)).convert('RGB')))
            masks_list.append(np.array(Image.open(os.path.join(path,'sorted masks',item)).convert('L')))
            print(item)
...
path='/content/ISIC2017_Segmentation_Test_Sorted'
name='a'
X_test, y_test = load_data(path,name)

from keras.models import load_model
model.load_weights('/content/drive/MyDrive/UNet_ISIC17_Focalmodulation_Skip14-11-24.h5')
...
for i in range(600):
  plt.imsave("/content/drive/MyDrive/UNet_ISIC17_Focalmodulation_Skip14_11_24/"+str(i+1)+".png",preds_test[i,:,:,0],cmap='gray') # binary segmenation